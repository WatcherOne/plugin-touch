export default {
    install: (app) => {
        const typeList = ['tap', 'longTap', 'swipe', 'swipeup', 'swipedown', 'swipeleft', 'swiperight']
...
vueTouch.prototype = {
    start: function (e) {
        ...
        if (disX > 10 || disY > 100) {
            this.touchType === 'swipe' && this.callBack(e)
            // 移动相对水平方向角度 < 45° 属于水平移动
            if (disX > disY) {
                // 右移动
                if (moveX > 10) this.touchType === 'swiperight' && this.callBack(e)
                // 左移动
                if (moveX < -10) this.touchType === 'swipeleft' && this.callBack(e)
            } else { // 属于垂直移动
                // 下移动
                if (moveY > 10) this.touchType === 'swipedown' && this.callBack(e)
                // 上移动
                if (moveY < -10) this.touchType === 'swipeup' && this.callBack(e)
            }
        } else {
